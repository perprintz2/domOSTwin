...
function draw_logo(ctx, id) {
    ...
    imageObjGui.onload = function () {
        //ctx.clearRect(0, 0, 900, 600);
         ...
         //ctx.font = "bold " + 11 + "px Arial";
         ctx.fillStyle = "#000000";
         ctx.fillText(": Simulated version of the real house. (Neural Network based)", 230,    83,);
         ctx.fillText("- Click on the individual meter in the top right window", 70,    163,);
         ctx.fillText("to display the graph in the window below.", 85,    193,);
         ctx.fillText("- Click on the houses in the top left window", 70,    463,);
         ctx.fillText("to select the individual house.", 85,    493,);
         ctx.stroke();
        ctx.drawImage(imageObjGui, 85, 230, 460, 179);
    };
    ...
    imageObjDT.onload = function () {
        ...
        ctx.drawImage(imageObjDT, 50, 10,170, 107);
    };
...
function init_apartment_data_R13sim(id) {
    ...
    let size = 140;
    widgetsR13.push(new TempHumiCo2NN(id, "Kitchen",   70, 110, size));
    widgetsR13.push(new TempHumiCo2NN(id, "Room4",    375, 190, size));
...
function draw_apartment_gui_R13sim(ctx, id) {
    ...
    imageObj.onload = function () {
        ...
        ctx.font = "24.5px Times New Roman";
        ctx.fillStyle = "#000000";
        ctx.fillText(id+": -- Simulated temperatures --", 15, 27);
        ctx.font = "bold " + 11 + "px Arial";
        ctx.fillStyle = "#000000";
        ctx.fillText("ODE model", 1164,    223,);
